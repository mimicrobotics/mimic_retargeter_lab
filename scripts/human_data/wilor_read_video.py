...
def test_wilor_image_pipeline(test_images_dir: str):
    import cv2
    import torch
    ...
    img_paths = Path(test_images_dir).glob("*.jpg")
    outputs_for_saving = {}
    for img_path in img_paths:
        ...
        outputs_for_saving[img_path.name] = [
            outputs,
        ]
    ...
    with open(os.path.join(save_dir, "outputs.pkl"), "wb") as f:
        pickle.dump(outputs_for_saving, f)
...
def test_wilor_video_pipeline(render=False, save_pkl=True):
    import cv2
    import torch
    import numpy as np
    from pathlib import Path
    ...
    # video_path = "assets/test_hand.mp4"
    base_path = Path(__file__).resolve().parents[2] / "dataset/wilor"
    video_paths = base_path.glob("*.mp4")
    save_dir = "./results"
    os.makedirs(save_dir, exist_ok=True)
    renderer = Renderer(pipe.wilor_model.mano.faces)
    out_save = {}
    ...
    for video_path in video_paths:
        ...
        while cap.isOpened():
            ...
            if video_path not in out_save:
                out_save[video_path] = []
            out_save[video_path].append(outputs)
    ...
    with open("video_wilor.pkl", "wb") as f:
