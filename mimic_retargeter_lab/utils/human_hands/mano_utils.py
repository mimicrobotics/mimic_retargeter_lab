...
def get_mano_joints_dict(
    joints: np.ndarray, include_wrist=False, batch_processing=False
):
    # joints: 21 x 3
...
def get_keyvectors(fingertips: Dict[str, np.ndarray], palm: np.ndarray):
    return {
...
def get_hand_rotation_matrix(joint_pos, flip_x_axis=True, flip_y_axis=False):
    """
