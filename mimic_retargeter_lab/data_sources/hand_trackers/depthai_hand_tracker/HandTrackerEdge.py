...
import mimic_retargeter_lab.data_sources.hand_trackers.depthai_hand_tracker.mediapipe_utils as mpu
from mimic_retargeter_lab.data_sources.hand_trackers.depthai_hand_tracker.FPS import FPS
...
class HandTracker:
    ...
    def __init__(
        ...
        self.logger = logger
        self.logger.info("Running edge inference")
        self.use_lm = use_lm
        if not use_lm:
            self.logger.warning("use_lm=False is not supported in Edge mode.")
            sys.exit()
        self.pd_model = pd_model
        self.logger.info(f"Palm detection blob     : {self.pd_model}")
        if lm_model == "full":
        ...
        self.logger.info(f"Landmark blob           : {self.lm_model}")
        self.pd_score_thresh = pd_score_thresh
        self.pd_nms_thresh = pd_nms_thresh
        self.lm_score_thresh = lm_score_thresh
        self.pp_model = pp_model
        self.logger.info(f"PD post processing blob : {self.pp_model}")
        self.solo = solo
        if self.solo:
            self.logger.info("In Solo mode, # of landmark model threads is forced to 1")
        ...
        if input_src == None or input_src == "rgb" or input_src == "rgb_laconic":
            ...
            if resolution == "lite":
                ...
                self.logger.error(f"Error: {resolution} is not a valid resolution !")
                sys.exit()
            self.logger.info("Sensor resolution:", self.resolution)
            ...
            self.logger.info(f"Internal camera FPS set to: {self.internal_fps}")
            ...
            self.logger.warning("Invalid input source:", input_src)
            sys.exit()
        ...
        self.logger.info(
            f"Pipeline started - USB speed: {str(usb_speed).split('.')[-1]}"
        )
    ...
    def create_pipeline(self):
        ...
        # ColorCamera
        self.logger.info("Creating Color Camera...")
        ...
        if self.xyz:
            ...
            self.logger.info(f"RGB calibration lens position: {calib_lens_pos}")
        ...
        self.logger.info("Creating Palm Detection pre processing image manip...")
        ...
        self.logger.info("Creating Palm Detection Neural Network...")
        pd_nn = pipeline.create(dai.node.NeuralNetwork)
        pd_nn.setBlobPath(self.pd_model)
        pre_pd_manip.out.link(pd_nn.input)

        # Define pose detection post processing "model"
        self.logger.info("Creating Palm Detection post processing Neural Network...")
        ...
        self.logger.info("Creating Hand Landmark pre processing image manip...")
